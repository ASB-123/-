...
def backtest_symbol(symbol, df5, df15):
    ...
    for i in range(122, len(df5)):
        ...
        bull_ma    = prev["ema20"] > prev["ema60"] > prev["ema120"]
        bear_ma    = prev["ema20"] < prev["ema60"] < prev["ema120"]
        macd_cross = prev["hist"] > 0 and prev2["hist"] <= 0   # 金叉
        macd_death = prev["hist"] < 0 and prev2["hist"] >= 0   # 死叉

        for side, cond in [("BUY", bull_ma and macd_cross), ("SELL", bear_ma and macd_death)]:
            if not cond:
                continue
...
def print_results(all_trades, dfs):
    ...
    print(f"\n{'='*68}")
    print(f"{' / '.join(SYMBOLS)}  5m進場 / 15m VP TP  多空雙向")
    print(f"名義倉位：{NOTIONAL} USDT/筆  SL：ATR({ATR_PERIOD})×{ATR_MULTIPLIER}  TP1≥{TP1_MIN_PCT*100}%")
